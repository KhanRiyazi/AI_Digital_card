...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting Smart Student ID Card API...")
    # Load ML model if exists
    model_path = os.getenv("MODEL_PATH", "./app/ml_models/saved_models/student_performance.pkl")
    if os.path.exists(model_path):
        try:
            ml_pipeline.load_model(model_path)
            logger.info("ML model loaded successfully")
        except Exception as e:
            logger.warning(f"Could not load ML model: {e}")
    else:
        logger.info("No pre-trained model found. Using fallback predictions.")
...
app = FastAPI(
    title="Smart Student ID Card API",
...
async def root():
    return {
        "message": "Smart Student ID Card API",
        "status": "running",
        "version": "2.0.0"
    }
...
async def health_check():
    return {
        "status": "healthy",
        "ml_model_loaded": ml_pipeline.is_trained
    }
